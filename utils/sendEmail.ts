...
export const sendEmailsToContacts = async (
    ...
    console.log(`Starting bulk email process for ${data.contacts.length} contacts.`);
    ...
    for (let i = 0; i < data.contacts.length; i++) {
        ...
        console.log(`\n--- Processing contact ${i + 1}/${data.contacts.length}: ${contact.name} (${contact.email}) ---`);
        ...
        try {
            ...
            console.error(`Failed to send email to ${contact.email}. Continuing to next contact.`);
        ...
        if (i < data.contacts.length - 1) {
            console.log(`Waiting for ${delayBetweenEmailsMs}ms before processing next contact...`);
